...
\renewcommand{\abstractname}{$if(abstract_name)$$abstract_name$$else$Abstract$endif$}
...
\author{ \vspace{12pt} \\ $for(author)$ $if(author.name)$ $author.name$ $if(author.affiliation)$ \\ $author.affiliation$ \\ $endif$ $else$ $author$ $endif$ $sep$ \\ $endfor$
         \vspace{15pt} \\ $for(organization)$ $if(organization.name)$ \emph{ $organization.name$ } $else$ \emph{ $organization$ } $endif$ $sep$ \\ $endfor$ \\ \vspace{25pt}}
