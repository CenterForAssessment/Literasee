...
Table `r tblNumNext()` shows the number of valid student records available for analysis after applying the data preparation business rules.^[This number does not represent the number of SGPs produced, however, because students are required to have at least one prior score available as well.]

```{r, cache=TRUE, echo=FALSE, include=FALSE}
	n_tbl_EOG <- table(sgp_object@Data[YEAR==sgp.year & CONTENT_AREA %in% GL_subjects & VALID_CASE=='VALID_CASE']$CONTENT_AREA, sgp_object@Data[YEAR==sgp.year & CONTENT_AREA %in% GL_subjects & VALID_CASE=='VALID_CASE']$GRADE)
```
...
```{r, cache=TRUE, echo=FALSE, include=FALSE}
	n_tbl_EOC <- table(sgp_object@Data[YEAR==sgp.year & CONTENT_AREA %in% EOC_subjects & VALID_CASE=='VALID_CASE']$CONTENT_AREA, sgp_object@Data[YEAR==sgp.year & CONTENT_AREA %in% EOC_subjects & VALID_CASE=='VALID_CASE']$GRADE)
```
```{r, results='asis', echo=FALSE, N_tableEOC}
	n_tbl_EOC_B <- n_tbl_EOC[match(EOC_subjects, row.names(n_tbl_EOC)) ,]
	n_tbl_EOC_B <- cbind('Content Area'=sapply(EOC_subjects, SGP::capwords, special.words=c("ELA", "II", "US")), 'Valid Records' = n_tbl_EOC_B)
  ...
  cat(dualTable(as.matrix(n_tbl_EOC_B), align=paste(rep('r', dim(n_tbl_EOC_B)[2]), collapse=''), 
		caption='Total Number of Valid EOC Student Records by Subject'))
